...
class Core(object):
    ...
    def __init__(self, MSGR):
        ...
        self.mdldr = ModuleLoader(
            self.BOT_MODULES, self.modules_path, self.LOADED_MODULES
        )
    ...
    def _init_static(self) -> None:
        ...
        try:
            ...
            with open(self.wb_config_path, "w", encoding="utf-8") as f:
                init_config = {
                    "botName": "YOUR TRIGGER FOR GROUPCHAT",
...
class Handler(object):
    ...
    def handle_status_msg(self, msgJson) -> None:
        ...
        # WindBot is Invited into a new Groupchat
            ...
            self.msgr.send_txt_msg(
                self.wb_invite_repo + from_id, wxid=self.SUDO_LIST[0]
            )
    ...
    def handle_pat_wb(self, msgJson):
        ...
        if from_id.endswith("@chatroom"):
            ...
            wxid_query = self.wb_db.fetch(
                f"r{room_num}", ["wxid"], "groupUsrName", usr_nickname
            )
        ...
        rec_pat_times = self.wb_db.fetch("Users", ["patTimes"], "wxid", usr_id)[0][0]
        new_pat_times = rec_pat_times + 1
        self.wb_db.update("Users", "patTimes", new_pat_times, "wxid", usr_id)
        ...
        ## Trigger PatAction
        pat_data = self.wb_db.fetch("Users", ["patAction"], "wxid", usr_id)[0][0]
        # If user did not bind any action
        if pat_data == "-1":
            reply = "您没有绑定PatAction指令。\n"
            reply += f"示例绑定: {self.BOT_GC_INVOKER} bind pat mb50\n"
            reply += (
                f"如果您不想再看到这条信息，请使用 {self.BOT_GC_INVOKER} bind pat nop"
            )
    ...
    def handle_recv_msg(self, msgJson) -> None:
        ...
        if msgJson.get("refnick", -1) != -1 and msgJson.get("refcontent", -1) != -1:
            isCite = True
        ...
        if msgJson["wxid"].endswith("@chatroom"):
            ...
            nickname = self.wb_db.fetch(
                f"r{room_num}", ["groupUsrName"], "wxid", sender_id
            )[0][0]

            roomname = self.wb_db.fetch(
        ...
        # If msg comes from DM
            ...
            nickname = self.wb_db.fetch("Users", ["realUsrName"], "wxid", sender_id)[0][
                0
            ]
    ...
    def handle_recv_keyword(self, keyword, destination, isRoom) -> None:  # @TODO
        ...
        if keyword == "help":
            ...
            self.msgr.send_txt_msg(random.choice(self.wb_summon_msg), wxid=destination)
    ...
    def pre_call(self, func_keyword, func_data, usr_id, destination) -> None:
        ...
        if func_keyword in self.depreciated_func_msg:
            self.msgr.send_txt_msg(self.depreciated_func_msg[func_keyword], destination)
            return
        ...
        if func_keyword not in self.all_func:
            ...
            self.msgr.send_txt_msg(func_non_existent_msg, destination)
            return
        ...
        for module in self.avail_usr_func:
            if func_keyword in self.avail_usr_func[module]:
                ...
                if func_status == False:
                    self.msgr.send_txt_msg(self.func_disabled_msg, destination)
                    return
                ...
                ## Provide Access to WB Data for the Maimai Module.
                    ...
                    execute_args.append([self.wb_db, self.BOT_GC_INVOKER])

                ######## WATERPROOF TAPE PATCH ########

                # User Functions will be executed in threads
                tFunc = Thread(
                    target=self.execute_call, args=(func_keyword, execute_args, True)
                )
                tFunc.start()
                return
        ...
        for module in self.avail_mngng_func:
            if func_keyword in self.avail_mngng_func[module]:
                ...
                if self.power_check(usr_id, 3) == False:
                    self.msgr.send_txt_msg(self.power_weak_msg, destination)
                    return
                ...
                self.execute_call(func_keyword, execute_args, False)
                return
    ...
    def banned_check(self, wxid) -> bool:
        caller_ban_status = self.wb_db.fetch("Users", ["banned"], "wxid", wxid)[0][0]
    ...
    def power_check(self, wxid, req_power) -> bool:
        caller_lvl = self.wb_db.fetch("Users", ["powerLevel"], "wxid", wxid)[0][0]
    ...
    def handle_hb(self, j) -> None:
        ...
        if self.undisturbed_hb < 5:
            ...
            output(
                "Undisturbed in 5 min. Hiding heartbeat logs. zZZ",
                logtype="HEART_BEAT",
                mode="HIGHLIGHT",
            )
        ...
        for i, item in enumerate(j["content"]):
            ...
            if item["wxid"].endswith("@chatroom"):
                ...
                # Exists, update groupchat infomation
                    ...
                    self.wb_db.update(
                        "Groupchats", "groupname", group_name, "roomid", room_id
                    )
    ...
    def handle_chat_nick(self, j) -> None:
        ...
        self.wb_db.update(f"r{room_id}", "groupUsrName", nickname, "wxid", wxid)
...
class ModuleLoader(object):
    ...
    def scan_module(self) -> None:
        ...
        for item in os.scandir(self.modules_path):
            if item.is_dir():
                ...
                for sub_item in os.scandir(item.path):
                    if sub_item.is_file():
                        if sub_item.name == f"{self.std_filename}.py":
                            output(f"Found Module [{item.name}]", background="WHITE")
                            main_found = True
                ...
                if not main_found:
                    output(
                        f"No main.py found in module folder '{item.name}'",
                        "WARNING",
                        background="WHITE",
                    )
    ...
    def load_module(self, module_name, file_path) -> list:
        ...
        output(f"Loaded Module {name} [Ver {ver}] by {author_str}", background="WHITE")
