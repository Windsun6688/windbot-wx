...
def pjskpf(datalist, callerid, roomid=None):
    ...
    if usr_data == {}:
        return [
            f"API未返回任何数据。这可能是网络原因，也可能是绑定错误。\n您现在绑定的ID: {userid}"
        ]
...
def pjsk_event_get(local: bool = False):
    ...
    if local:
        with open(
            os.path.join(resource, "event_data.json"), "r", encoding="utf-8"
        ) as f:
        ...
        if resp.status_code != 200:
            ...
            data = resp.json()
            with open(
                os.path.join(resource, "event_data.json"), "w", encoding="utf-8"
            ) as f:
...
def load_event_info(_data):
    ...
    return (
        ...
        _data[i]["name"],
        time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(int(_data[i]["aggregateAt"] / 1000))
        ),
        _data[i]["eventType"],
        _data[i]["unit"],
        bool(i == -1),
...
def pjsk_music_get(local: bool = False):
    ...
    if local:
        with open(
            os.path.join(resource, "music_data.json"), "r", encoding="utf-8"
        ) as f:
            data = json.loads(f.read())
    else:
        ...
        if resp.status_code != 200:
            ...
            data = resp.json()
            with open(
                os.path.join(resource, "music_data.json"), "w", encoding="utf-8"
            ) as f:
                f.write(json.dumps(data, ensure_ascii=False, indent=4))
    return data, status
...
def pjsk_music_search(datalist, callerid, roomid=None):
    ...
    for s in results:
        ...
        publishedAt = time.strftime(
            "%Y-%m-%d", time.localtime(int(s["publishedAt"] / 1000))
        )
        genre = s["genre"]
        bpm = s["bpm"]
        musicId = s["musicId"]
...
def pjsk_chart_get(local: bool = False):
    ...
    if local:
        with open(
            os.path.join(resource, "chart_data.json"), "r", encoding="utf-8"
        ) as f:
            data = json.loads(f.read())
    else:
        ...
        if resp.status_code != 200:
            ...
            data = resp.json()
            with open(
                os.path.join(resource, "chart_data.json"), "w", encoding="utf-8"
            ) as f:
                f.write(json.dumps(data, ensure_ascii=False, indent=4))
    return data, status
