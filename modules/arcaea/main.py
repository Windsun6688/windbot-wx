...
class Arcaea(object):
    ...
    # Fetch Data from the Arcaea Wiki.
    def _arc_wiki_data_get(self, item: str):
        """
        page Arc中文维基页面名称:
    ...
    # Fetch or Get Locally the music data.
    def _music_get(self, local: bool) -> Tuple[dict, bool]:
        ...
        success = True
        if not local:
            resp = self._arc_wiki_data_get("songlist")

            if isinstance(resp, dict):
                song_dict = resp["parse"]["wikitext"]["*"]
                with open(
                    os.path.join(self.STATIC_PATH, "song_dict.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
                    f.write(song_dict)
                    f.close()
                song_dict = json.loads(song_dict)
            else:
                mh.output(
                    "Arcaea曲目数据获取失败,切换至本地暂存文件",
                    "WARNING",
                    background="WHITE",
                )
                local = True
        ...
        if local:
            with open(
                os.path.join(self.STATIC_PATH, "song_dict.json"), "r", encoding="utf-8"
            ) as f:
                song_dict = json.loads(f.read())

        # To handle deleted songs.
        song_dict["songs"] = [
            s for s in song_dict["songs"] if not (s.get("deleted", False))
        ]

        return (song_dict, success)

    # Fetch or Get Locally the chart data.
    def _chart_get(self, local: bool) -> Tuple[dict, bool]:
        """
        ...
        success = True
        if not local:
            resp = self._arc_wiki_data_get("constant")

            if isinstance(resp, dict):
                const_dict = resp["parse"]["wikitext"]["*"]
                with open(
                    os.path.join(self.STATIC_PATH, "const_dict.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
                    f.write(const_dict)
        ...
        if local:
            with open(
                os.path.join(self.STATIC_PATH, "const_dict.json"), "r", encoding="utf-8"
            ) as f:
                const_dict = json.loads(f.read())

        return (const_dict, success)

    # Fetch or Get Locally the complex artist breakdown data.
    def _complex_artist_get(self, local: bool) -> Tuple[dict, bool]:
        """
        Not Local: Get Complex Artist Data From Arcaea Wiki
        Local: Read Complex Artist Data From Local File
        """

        success = True
        if not local:
            resp = self._arc_wiki_data_get("artistlist")

            if isinstance(resp, dict):
                complex_artist_data = resp["parse"]["wikitext"]["*"]
                with open(
                    os.path.join(self.STATIC_PATH, "complex_artist.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
        ...
        if local:
            with open(
                ...
                complex_artist_data = json.loads(f.read())

        return (complex_artist_data, success)

    # Fetch or Get Locally the song length data.
    def _music_length_get(self, local: bool) -> Tuple[dict, bool]:
        """
        Not Local: Get Song Length Data From Arcaea Wiki
        Local: Read Song Length Data From Local File
        """

        success = True
        if not local:
            resp = self._arc_wiki_data_get("songlen")

            if isinstance(resp, dict):
                song_len_data = resp["parse"]["wikitext"]["*"]
                with open(
                    os.path.join(self.STATIC_PATH, "song_len.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
        ...
        if local:
            with open(
                os.path.join(self.STATIC_PATH, "song_len.json"), "r", encoding="utf-8"
            ) as f:
                song_len_data = json.loads(f.read())

        return (song_len_data, success)

    # Fetch or Get Locally the version update date data.
    def _ver_time_get(self, local: bool) -> Tuple[dict, bool]:
        """
        Not Local: Get Version Update Date Data From Arcaea Wiki
        Local: Read Version Update Date Data From Local File
        """

        success = True
        if not local:
            resp = self._arc_wiki_data_get("versiontime")

            if isinstance(resp, dict):
                version_time_data = resp["parse"]["wikitext"]["*"]
                with open(
                    os.path.join(self.STATIC_PATH, "version_time.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
        ...
        if local:
            with open(
                ...
                version_time_data = json.loads(f.read())
    ...
    def _pack_get(self, local: bool) -> Tuple[dict, bool]:
        ...
        if not local:
            ...
            if isinstance(resp, dict):
                ...
                with open(
                    os.path.join(self.STATIC_PATH, "pack_dict.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
                    f.write(pack_data)
        ...
        if local:
            with open(
                os.path.join(self.STATIC_PATH, "pack_dict.json"), "r", encoding="utf-8"
            ) as f:
    ...
    def _unlock_get(self, local: bool) -> Tuple[dict, bool]:
        ...
        if not local:
            ...
            if isinstance(resp, dict):
                ...
                with open(
                    os.path.join(self.STATIC_PATH, "unlocks.json"),
                    "w",
                    encoding="utf-8",
                ) as f:
                    f.write(unlock_data)
        ...
        if local:
            with open(
                os.path.join(self.STATIC_PATH, "unlocks.json"), "r", encoding="utf-8"
            ) as f:
    ...
    def static_update(self, args: list) -> dict:
        ...
        resp += (
            f"复合曲师数据: {status[int(self._complex_artist_get(local = False)[1])]}\n"
        )
        resp += f"版本时间数据: {status[int(self._ver_time_get(local = False)[1])]}\n"
        return mh.compose_txt_msg(resp)

    # Retrieves all songs in a given difficulty.
                        ...
                        if chart != None and chart["constant"] == float(diff):
                            ...
                            for song_data in music_data:
                                if song_data["id"] == song_id:
                                    ...
                                    title_ja = song_data["title_localized"].get(
                                        "ja", None
                                    )
                                    if title_ja != None:
                                        title += f"({title_ja})"
                                    ...
                                    for difficulty in song_data["difficulties"]:
                                        if difficulty["ratingClass"] == i:
                                            charter = difficulty["chartDesigner"]

                                    reply += f"[{self.DIFF_LIST[i]}] "
                                    reply += f"{artist} - {title} <{charter}>\n"
                if found_cnt == 0:
                    reply = f"WB没有在{diff}难度找到谱面。"
                else:
                    reply += f"共{found_cnt}张谱面。"
    ...
    # Music data by song_id.
    def _music_by_song_id(self, song_id) -> list:
        music_data = self._music_get(local=True)[0]["songs"]
        result = []
        for song in music_data:
            if song["id"] == song_id:
    ...
    def music_search(self, args) -> dict:
        ...
        # Fuzzy Search
        if func_data[0].lower() == "f":
            keyword = " ".join(func_data[1:])
            results = self._music_by_fuzzy_title(keyword, fuzzy_rate)
        # Precise Search
        else:
        ...
        # Too Many Results
            ...
            reply = "WB找到了太多结果，搬不回来了。请尝试优化搜索词。"
