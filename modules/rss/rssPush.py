...
rss_subscriptions = [
    ["biliDynamic", "404145357"],  # Arcaea BiliDynamic\
...
def check_rss(route: str, usrID: str = None):
    ...
    while True:
        ...
        if first_feed_index == len(feed_data[route + usrID]["rss"]["channel"]["item"]):
        ...
        if (
            listjson["rss"]["channel"]["item"][0]["link"]
            != feed_data[route + usrID]["rss"]["channel"]["item"][first_feed_index][
                "link"
            ]
        ):
            # 先去找原来的第一个在不在新的里面
