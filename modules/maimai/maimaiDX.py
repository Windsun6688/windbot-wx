...
if dev_token == "":
    output("Continuing Without Developer Token", "WARNING", background="WHITE")

MAI_API = "https://www.diving-fish.com/api/maimaidxprober"
MAI_ALIAS_API = "https://api.yuzuchan.moe/maimaidx"
...
def mai_music_get(local: bool = False) -> Tuple[dict, bool]:
    ...
    if not local:
        ...
        if isinstance(songs, list):
            with open(
                os.path.join(static, "music_data.json"), "w", encoding="utf-8"
            ) as f:
...
def mai_chartstats_get(local: bool = False) -> Tuple[dict, bool]:
    ...
    if not local:
        ...
        if isinstance(chart_stats, dict):
            with open(
                os.path.join(static, "chart_stats.json"), "w", encoding="utf-8"
            ) as f:
...
def mai_alias_get(local: bool = False) -> Tuple[dict, bool]:
    ...
    if not local:
        ...
        if isinstance(alias, dict):
            with open(
                os.path.join(static, "song_alias.json"), "w", encoding="utf-8"
            ) as f:
...
def mai_plate_check(usr_plate_data: list, versions: list, plan: int):
        ...
        if song_version in versions:
            ...
            for ds in song["ds"]:
                if song_id in remaining[diff_cnt]:
                    if ds > 13.6:
                        remaining_hard.append(
                            (song_id, song_title, diffs[diff_cnt], ds)
                        )
    ...
    remaining_hard = sorted(remaining_hard, key=lambda i: i[3], reverse=True)
...
def mai_single_score(gamertag: str, sid: int):
    ...
    usr_single_grade = [
        grade for grade in usr_data["records"] if grade["song_id"] == sid
    ]
...
def mai_music_search(datalist, callerid, roomid=None):
    ...
    # Other Searches
        ...
        if keyword.isnumeric():
            results = mai_music_by_id(int(keyword))
    ...
    if isinstance(results, list):
        ...
        return [f"没有搜寻到结果。您查找了SID: {keyword}"]
...
def mai_alias_search(datalist, callerid, roomid=None):
    ...
    for song in song_alias["content"]:
        if keyword in song["Alias"]:
            results.append((song["SongID"], song["Name"]))
...
def computeRa(
    ds: float, achievement: float, spp: bool = False, israte: bool = False
...
def best_2_image(output: Image.Image, data: list, isOld: bool):
    ...
    # Load Assets
    dxstar = [
        Image.open(os.path.join(mai_dir, f"UI_RSL_DXScore_Star_0{_ + 1}.png")).resize(
            (20, 20)
        )
        for _ in range(3)
    ]
    ...
    for s in data:
        ...
        # Fixed
        try:
            cover = Image.open(os.path.join(cover_dir, f"{info[3]}.png")).resize(
                (135, 135)
            )
        except FileNotFoundError as e:
            cover = Image.open(
                os.path.join(cover_dir, f"{random.randint(1,2)*-1}.png")
            ).resize((135, 135))

        version = Image.open(
            os.path.join(mai_dir, f"UI_RSL_MBase_Parts_{info[5]}.png")
        ).resize((55, 19))
        rate = Image.open(os.path.join(mai_dir, f"UI_TTR_Rank_{info[9]}.png")).resize(
            (95, 44)
        )
        ...
        if info[11]:
            fc = Image.open(
                os.path.join(mai_dir, f"UI_MSS_MBase_Icon_{info[11]}.png")
            ).resize((45, 45))
        ...
        if info[12]:
            fs = Image.open(
                os.path.join(mai_dir, f"UI_MSS_MBase_Icon_{info[12]}.png")
            ).resize((45, 45))
        ...
        text_output.text(
            (x + 155, y + 80),
            f"Rating {info[4]} -> {computeRa(info[4], info[6], True)}",
            font=_tbRating,
            fill=TEXT_COLOR[info[2]],
            anchor="lm",
        )
...
def draw_best_image(gamertag: str):
    ...
    dx_rating = Image.open(
        os.path.join(mai_dir, rating_picture(ra + add_ra, b50=True))
    ).resize((425, 80))
    Name = Image.open(os.path.join(mai_dir, "Name.png"))
    MatchLevel = Image.open(os.path.join(mai_dir, friend_match_picture(add_ra))).resize(
        (128, 58)
    )
    rating = Image.open(os.path.join(mai_dir, "UI_CMN_Shougou_Rainbow.png")).resize(
        (454, 50)
    )
    ...
    if plate:
        ...
        plate = Image.open(os.path.join(mai_dir, "UI_Plate_300101.png")).resize(
            (1420, 230)
        )
    # Draw Plate
    ...
    # Write Split Rating
    text_im.text(
        (847, 300), "New Rating System", font=_siyuan, fill=(0, 0, 0, 255), anchor="mm"
    )

    # Write Credits
    text_im.text(
        (900, 2365),
        "Generated by WINDBOT | Assets&Design by Yuri-YuzuChaN & BlueDeer233",
        font=_tb,
        fill=(103, 20, 141, 255),
        anchor="mm",
    )
    ...
    for n, i in enumerate(total_ra):
        if n == 0 and i == 0:
            continue
        im.alpha_composite(
            Image.open(os.path.join(mai_dir, f"UI_NUM_Drating_{i}.png")),
            (820 + 33 * n, 133),
        )
